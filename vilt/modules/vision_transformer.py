...
class MoE(nn.Module):
    ...
    def forward(self, x):
        for layer in self.moe_layer_list:
            x, _, _ = layer(x)
            # TODO (output, loss, gate_values)其中，output是经过所有专家处理后的张量，loss是一个可能用于模型训练的额外损失项（例如，专家的负载均衡损失），gate_values是一个表示输入如何在不同专家间分配的张量。
        return x
...
class BlockForMoeViST(nn.Module):
    def __init__(
        self,
        dim,
        num_heads,
        ...
        self.norm_sensor = norm_layer(dim)
        self.attn = AttentionForViST(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            qk_scale=qk_scale,
    ...
    def forward(self, x_image, x_sensor):
        ...
        x_image = x_image + \
            self.drop_path(self.moe_image(self.norm2_image(x_image)))
        x_sensor = x_sensor + \
            self.drop_path(self.moe_sensor(self.norm2_sensor(x_sensor)))
        return x_image, x_sensor, attn_image, attn_sensor
...
class VisionTransformerForMoeViST(nn.Module):
    """ Vision Transformer
...
@register_model
def vit_base_patch32_384_SemanticEstimation(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/32) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=32, embed_dim=768,
                        depth=1, num_heads=12, **kwargs)
    model = _create_vision_transformer(
        "vit_base_patch32_384", pretrained=pretrained, **model_kwargs
    )
    return model
